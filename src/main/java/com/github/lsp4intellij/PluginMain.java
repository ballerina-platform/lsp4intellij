...
public class PluginMain {
    private Logger LOG = Logger.getInstance(PluginMain.class);

    private static final String SPLIT_CHAR = ";";

    private Map<Pair<String, String>, LanguageServerWrapper> extToLanguageWrapper = new HashMap<>();
    private Map<String, Set<LanguageServerWrapper>> projectToLanguageWrappers = new HashMap<>();
    private final Map<Pair<String, String>, LanguageServerWrapper> forcedAssociationsInstances = new HashMap<>();
    private Map<Pair<String, String>, LanguageServerDefinition> forcedAssociations = new HashMap<>();
    private final Object forcedAssociations_LOCK = new Object(){};
    private final Object forcedAssociationsInstances_LOCK = new Object(){};
    private Map<String, LanguageServerDefinition> extToServerDefinition = new HashMap<>();
    private boolean loadedExtensions = false;


    public void resetAssociations() {
        synchronized (forcedAssociationsInstances_LOCK) {
            forcedAssociationsInstances.forEach((s, languageServerWrapper) -> languageServerWrapper.disconnect(s.getKey()));
            forcedAssociationsInstances.clear();
        }
        synchronized (forcedAssociations_LOCK) {
            forcedAssociations.clear();
            Map<String[], String[]> collect = forcedAssociations.entrySet().stream()
                    .collect(Collectors.toMap(e -> new String[]{e.getKey().getKey(), e.getKey().getValue()},
     ...
     */
    public Set<LanguageServerWrapper> getAllServerWrappers() {
     ...
     */
    public void setExtToServerDefinition(Map<String, LanguageServerDefinition> newExt) {
        List<Map.Entry<String, LanguageServerDefinition>> nullDef = newExt.entrySet().stream().filter(
                d -> d.getValue() == null).collect(Collectors.toList());
        Map<String, LanguageServerDefinition> oldServerDef = extToServerDefinition;
        Map<String, LanguageServerDefinition> flattened = newExt.entrySet().stream()
        ...
        ApplicationUtils.pool(() -> {
            Set<String> added = flattened.keySet().stream().filter(e -> !oldServerDef.keySet().contains(e)).collect(
                    Collectors.toSet());
            Set<String> removed = oldServerDef.keySet().stream().filter(e -> !flattened.keySet().contains(e)).collect(
                    Collectors.toSet());
            synchronized (forcedAssociations_LOCK) {
                synchronized (forcedAssociationsInstances_LOCK) {
                    Set<LanguageServerDefinition> newValues = flattened.values().stream().collect(Collectors.toSet());
                    forcedAssociations = forcedAssociations.entrySet().stream().filter(
                            t -> newValues.contains(t.getValue())).collect(
                            Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
                    forcedAssociationsInstances.entrySet().stream().filter(
                            t -> !newValues.contains(t.getValue().getServerDefinition())).collect(
                            Collectors.toMap(e -> e.getKey(), e -> e.getValue())).keySet().forEach(k -> {
            ...
            List<Editor> openedEditors = ApplicationUtils.computableReadAction(() -> Arrays.stream(
            ...
            List<VirtualFile> files = openedEditors.stream().map(
                    e -> FileDocumentManager.getInstance().getFile(e.getDocument())).collect(
                    Collectors.toList());
    ...
    public void forceEditorOpened(Editor editor, LanguageServerDefinition serverDefinition, Project project) {
        addExtensions();
        VirtualFile file = FileDocumentManager.getInstance().getFile(editor.getDocument());
        if (file != null) {
            String uri = FileUtils.editorToURIString(editor);
            ...
            LanguageServerWrapper wrapper = getWrapperFor(serverDefinition.ext, editor, serverDefinition);
                Map<String[], String[]> associations = forcedAssociations.entrySet().stream()
                        .collect(Collectors.toMap(e -> new String[]{e.getKey().getKey(), e.getKey().getValue()},
                                                  e -> e.getValue().toArray()));
    ...
    private LanguageServerWrapper getWrapperFor(String ext, Editor editor, LanguageServerDefinition serverDefinition) {
        ...
        synchronized (forcedAssociationsInstances) {
            LanguageServerWrapper wrapper = forcedAssociationsInstances.get(
                    new ImmutablePair<>(FileUtils.editorToURIString(editor), FileUtils.projectToUri(project)));
                ...
                synchronized (extToLanguageWrapper) {
                    ...
                    if (wrapper == null) {
                        ...
                        Stream.of(exts).forEach(
                                ext2 -> extToLanguageWrapper.put(new ImmutablePair<>(ext2, rootUri), wrapper));
                    ...
                    synchronized (forcedAssociationsInstances) {
                        forcedAssociations.entrySet().forEach(t -> {
                            if (t.getValue() == serverDefinition && t.getKey().getValue().equals(rootUri)) {
                                forcedAssociationsInstances.put(t.getKey(), wrapper);
                            }
                        });
                        forcedAssociationsInstances.put(
                                new ImmutablePair<>(FileUtils.editorToURIString(editor), rootUri), wrapper);
                    }
                    return wrapper;
                }
            } else {
    ...
    private void addExtensions() {
        if (!loadedExtensions) {
            List<LanguageServerDefinition>
                    extensions = LanguageServerDefinition.getInstance().getAllDefinitions().stream().filter(
                    s -> !extToServerDefinition.keySet().contains(s.ext)).collect(Collectors.toList());
    ...
    private void flattenExt() {
        extToServerDefinition = extToServerDefinition.entrySet().stream()
                .flatMap(p -> {
                    String ext = p.getKey();
                    LanguageServerDefinition sDef = p.getValue();
                    String[] split = ext.split(SPLIT_CHAR);
                    Stream<AbstractMap.SimpleEntry<String, LanguageServerDefinition>> stream = Stream.of(
                            split).map(s -> new AbstractMap.SimpleEntry<>(s, sDef));
...
//    }
//
//    pubic void removeWrapper(LanguageServerWrapper wrapper) {
//        extToLanguageWrapper.remove((wrapper.getServerDefinition.ext, FileUtils.pathToUri(wrapper.getProject.getBasePath)))
//    }
//
//    public void setForcedAssociations(associations: java.util.Map[Array[String], Array[String]]) {
